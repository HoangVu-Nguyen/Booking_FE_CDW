...
import { ConversationSummaryResponse, MessageResponse, SendMessageRequest } from '../../models/response/chat.response';
...
export class ChatService {
   ...
   */
  loadChatHistory(conversationId: number, cursor?: number, limit: number = 20) {
    let params: any = { limit };
    if (cursor) params.cursor = cursor;

    this.apiService.get<ApiResponse<MessageResponse[]>>(`/api/v1/chat/conversations/${conversationId}/messages`, params)
      .subscribe({
        next: (res) => {
          if (res.data) {
            if (cursor) {
              // Tải thêm tin cũ: Nối mảng mới vào ĐẦU mảng hiện tại
              this.activeMessages.update(msgs => [...res.data, ...msgs]);
            } else {
              // Tải mới hoàn toàn: Ghi đè Signal và set Active ID
